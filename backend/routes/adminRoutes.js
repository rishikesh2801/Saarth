...
router.get('/tourists', auth, roleCheck(['POLICE', 'AIRPORT']), async (req, res) => {
  ...
  try {
    ...
    // MASTER OVERRIDE: If National Command Center, ignore ALL filters
    ...
    const tourists = await User.find(filter, 'name username lastLocation origin state safetyScore panicActive destination district originalPassword routePlan activeTrips updatedAt profilePic').sort({ updatedAt: -1 }).limit(1000);
    res.json(tourists);
  } catch (error) {
...
// Get Incoming Tourists (Destination matches, but current location doesn't)
router.get('/incoming', auth, roleCheck(['POLICE', 'AIRPORT']), async (req, res) => {
  try {
    ...
    const filter = { role: 'TOURIST' };
    console.log(`[INCOMING] Request from: ${req.user.username} | District: ${policeDistrict}`);
    
    if (policeState !== 'All India') {
        ...
        // Just for All India view, skip for now or show all
        filter.destination = { $ne: 'Unknown' };
    }

    console.log(`[INCOMING] Filter:`, filter);
    const tourists = await User.find(filter, 'name username lastLocation origin state destination district routePlan activeTrips updatedAt profilePic phone').sort({ updatedAt: -1 }).limit(100);
...
router.get('/stats', auth, roleCheck(['POLICE', 'AIRPORT']), async (req, res) => {
  try {
    ...
    const sosCount = await User.countDocuments({ ...filter, $or: [{ safetyScore: 0 }, { panicActive: true }] });
...
router.get('/search', auth, roleCheck(['POLICE', 'AIRPORT']), async (req, res) => {
  try {
    ...
    const filter = { role: 'TOURIST', username: id };
    ...
    const tourist = await User.findOne(filter, 'name username lastLocation origin state safetyScore panicActive destination district originalPassword routePlan activeTrips updatedAt profilePic');
    res.json(tourist);
  } catch (error) {
    res.status(500).json({ error: 'Server error' });
  }
...
router.post('/fine', auth, roleCheck(['POLICE']), async (req, res) => {
  try {
    ...
    const filter = { username: touristId };
    if (req.user.role === 'POLICE' && req.user.state === 'All India') {
       // Allow
    } else if (req.user.state) {
       filter.state = req.user.state;
    ...
    res.json({ success: true, message: 'Fine issued', user });
  } catch (error) {
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.post('/pay-fine', auth, roleCheck(['POLICE']), async (req, res) => {
  try {
    ...
    // We only allow police of the same state to manage fines, or All India
    const filter = { username: touristId };
    if (req.user.role === 'POLICE' && req.user.state !== 'All India') {
       if (req.user.district) filter.district = req.user.district;
       else filter.state = req.user.state;
    }
    ...
    res.json({ success: true, message: 'Fine marked as paid and cleared', user });
  } catch (error) {
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.post('/resolve-sos', auth, roleCheck(['POLICE']), async (req, res) => {
  try {
    ...
    const user = await User.findOneAndUpdate(
      filter,
      { panicActive: false, panicReason: '' },
      { new: true }
    );
    res.json({ success: true, message: 'SOS marked as resolved', user });
  } catch (error) {
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.post('/efir', auth, roleCheck(['POLICE']), async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate(
      filter,
      { $push: { efirs: { caseId, details } } }
    );

    res.json({ success: true, caseId, message: 'E-FIR generated successfully', details });
  } catch (error) {
    res.status(500).json({ error: 'Server error' });
  }
});

// Post a new global alert (Weather/Disaster)
router.post('/alerts', auth, roleCheck(['POLICE']), async (req, res) => {
  try {
    const { title, description, location, severity, type, scope } = req.body;
    
    let targetState = req.user.state;
    let targetDistrict = req.user.district;

    if (scope === 'STATE') {
      targetDistrict = null; // Broadcast to entire state
    } else if (scope === 'NATIONAL') {
      targetState = 'All India';
      targetDistrict = null; // Broadcast to entire country
    }

    // Automatically tag the alert with the calculated jurisdiction
    const alertData = { 
      title, 
      description, 
